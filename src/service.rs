...
use crate::runner::{self, Run, RunStart};
...
impl Service {
    ...
    /// paid-for reply when its client goes away should spawn it.
    pub async fn send<R: Run>(
        ...
        let session = self.session(user, session_id).await?;
        let _turn = self.store.lock_session(&session.id).await;
        self.store.begin_turn(&session.id);
        let started_at = now_millis();

        let outcome = agent.run(text, &session.id).await;
